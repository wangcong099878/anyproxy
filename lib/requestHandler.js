  ...
  return function (req, userRes) {
    ...
    */
    ...
    co(fetchReqData)
      ...
      // 发送请求前调用规则  invoke rule before sending request
...
function getConnectReqHandler(userRule, recorder, httpsServerMgr) {
  ...
  return function (req, cltSocket, head) {
    ...
    */
    co(function *() {
      console.log('1.来来来');
      // 确定是否使用中间人服务器  determine whether to use the man-in-the-middle server
      logUtil.printLog(color.green('received https CONNECT request ' + host));
      requestDetail = {
        host: req.url,
        _req: req
      };
      // 默认规则中的返回值为空 the return value in default rule is null
      // 因此，如果值为null，则将其作为最终值 so if the value is null, will take it as final value
      shouldIntercept = yield userRule.beforeDealHttpsRequest(requestDetail);

      // 否则，将采用传入选项 otherwise, will take the passed in option
      ...
      .then(() => {
        ...
        // 将套接字连接标记为已建立，以检测请求协议 mark socket connection as established, to detect the request protocol
